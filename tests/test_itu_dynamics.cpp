...
namespace {
    ...
    // closed-loop and open-loop sims stay double; compliance_dut<TypeParam>
    // runs the chain at TypeParam behind the double interface.
    using sample_types = ::testing::Types<float, double>;
    template <typename T>
    class itu_dynamics : public ::testing::Test {};
    TYPED_TEST_SUITE(itu_dynamics, sample_types);
    ...
    // matrix.)
    TYPED_TEST(itu_dynamics, SendActivationBuildUp) {
        for (const auto& rs : required_rates()) {
            ...
            measure<TypeParam>("SendActivation.ms", rs, t_hit_ms);
            EXPECT_LE(t_hit_ms, expected<TypeParam>({{25.0, 25.0}, {25.0, 25.0}}, rs)) << "fs " << rs.fs; // req <=50
        }
    }
    ...
    // within 0.5 s.
    TYPED_TEST(itu_dynamics, HangoverRecoveryAfterDoubleTalk) {
        for (const auto& rs : required_rates()) {
            compliance_dut<TypeParam>         c(chain_config<TypeParam>(rs), rs.block);
            ...
            // requirement.
            measure<TypeParam>("Hangover.0p5", rs, erl.by(t_end + 0.5));
            measure<TypeParam>("Hangover.1p0", rs, erl.by(t_end + 1.0));
            EXPECT_GE(erl.by(t_end + 0.5), expected<TypeParam>({{26.0, 15.0}, {26.0, 15.0}}, rs)) << "fs " << rs.fs;
            // The [20 dB within 1 s] provisional requirement.
            EXPECT_GE(erl.by(t_end + 1.0), expected<TypeParam>({{20.0, 20.0}, {20.0, 20.0}}, rs)) << "fs " << rs.fs;
        }
    }
    ...
    TYPED_TEST(itu_dynamics, TransmittedNoiseFluctuation) {
        for (const auto& rs : required_rates()) {
            compliance_dut<TypeParam> c(chain_config<TypeParam>(rs), rs.block);
            const auto                path = compliance_path(room::cabin, rs);
            ...
            measure<TypeParam>("NoiseFluctuation.span", rs, vmax - vmin);
            EXPECT_LE(vmax - vmin, expected<TypeParam>({{3.5, 3.5}, {3.5, 3.5}}, rs)) << "fs " << rs.fs; // req span<=6
        }
    }
    ...
    TYPED_TEST(itu_dynamics, ComfortNoiseLevelAndSpectrum) {
        for (const auto& rs : required_rates()) {
            compliance_dut<TypeParam> c(chain_config<TypeParam>(rs), rs.block);
            const auto                path   = compliance_path(room::cabin, rs);
            ...
            measure<TypeParam>("ComfortNoise.delta", rs, delta);
            EXPECT_LE(delta, expected<TypeParam>({{1.0, 1.0}, {1.0, 1.0}}, rs)) << "fs " << rs.fs;     // req +2
            EXPECT_GE(delta, expected<TypeParam>({{-2.5, -2.5}, {-2.5, -2.5}}, rs)) << "fs " << rs.fs; // req -5
    ...
    TYPED_TEST(itu_dynamics, NoisePumpingAcrossFarEndBursts) {
        for (const auto& rs : required_rates()) {
            compliance_dut<TypeParam> c(chain_config<TypeParam>(rs), rs.block);
            const auto                path  = compliance_path(room::cabin, rs);
            ...
            measure<TypeParam>("NoisePump.span", rs, lmax - lmin);
            EXPECT_LE(lmax - lmin, expected<TypeParam>({{10.0, 10.0}, {10.0, 10.0}}, rs)) << "fs " << rs.fs; // req <=10
        }
    }
    ...
    TYPED_TEST(itu_dynamics, ClosedLoopStabilitySweep) {
        for (const auto& rs : required_rates()) {
            const auto path = compliance_path(room::cabin, rs);
            css_config cc;
            cc.periods      = 14;
    ...
    TYPED_TEST(itu_dynamics, AlgorithmicDelayWithinBudget) {
        for (const auto& rs : required_rates()) {
