...
Runs every mutap_icount_* binary in a build directory under QEMU with the
...
by running once with --update in the target's CI environment and committing
...
def measure(target: str, plugin: str, binary: str) -> int:
    ...
    if "MUTAP_ICOUNT_DONE ok=1" not in out:
        print(out, file=sys.stderr)
        raise SystemExit(f"{binary}: workload did not complete cleanly")
    m = re.search(r"MUTAP_INSN_COUNT (\d+)", out)
    if not m:
        print(out, file=sys.stderr)
        raise SystemExit(f"{binary}: no MUTAP_INSN_COUNT (plugin not loaded?)")
...
def main() -> int:
    ...
    binaries = sorted(glob.glob(os.path.join(args.build_dir, "**", "mutap_icount_*"),
                                recursive=True))
    binaries = [b for b in binaries if os.access(b, os.X_OK) and os.path.isfile(b)]
    if not binaries:
        raise SystemExit(f"no mutap_icount_* binaries under {args.build_dir}")
    ...
    for binary in binaries:
        scenario = os.path.basename(binary).removeprefix("mutap_icount_")
